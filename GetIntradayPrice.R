...
myendpoints <- function(x,period){
  ...
  for (i in 1:n){
    xx <- index(x[paste(as.Date(dates[i]))]) 
...
my.to.period <- function(x,period){
  freq<-unclass(periodicity(x))
  if(freq$units != "mins")
    error("unrecognized frequency unit")
  ...
  cnames <- colnames(x)
  xx <- .Call("toPeriod", x, myendpoints(x,period), has.Vo(x), 
